...
import com.example.localapi.viewmodel.StatusUIState
...
fun HomeBody(
    ...
    Column(
        ...
        when (statusUISiswa) {
            ...
            is StatusUIState.Success -> DataSiswa(
                siswas = statusUISiswa.data,
                onSiswaClick = onSiswaClick
            )
            is StatusUIState.Error -> ErrorScreen(
...
@Composable
fun DataSiswa(
    siswas: List<DataSiswa>,
    onSiswaClick: (DataSiswa) -> Unit,
    modifier: Modifier = Modifier
) {
    LazyColumn(modifier = modifier) {
        items(items = siswas, key = { it.id }) { person ->
            ItemSiswa(
                siswa = person,
                modifier = Modifier
